...
#ifdef MIAL
/* cvimage - convert an image to an image node */
LVAL cvimage(IMAGE *i)
{
  LVAL val;
  if(i==NULL){
    xlfail("Try to convert a NULL pointer into an image node");
    return(NIL);
  }
    
  xlsave1(val);
  val = newnode(IMAGE_node);
  val->n_info.n_image = i;
  total += GetImNByte(i);
  xlpop();
  return(val);
}
...
LOCAL VOID NEAR sweep()
{
    ...
    for (seg = segs; seg != NULL; seg = seg->sg_next){
		...
		case IMAGE_node:
			if (getimage(p) != NULL){
			    total -= GetImNByte(getimage(p));
			    free_image(getimage(p));
			    p->n_info.n_image = NULL;
			  }
			break;			
#endif
#ifdef BIGNUMS
