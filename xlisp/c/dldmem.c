...
#define btow_size(n) (((unsigned)(n)+(sizeof(LVAL)-1))/sizeof(LVAL))
#define stow_size(n) (((unsigned)(n)*sizeof(BIGNUMDATA)+(sizeof(LVAL)-1))/sizeof(LVAL))
    ...
    if (vsfree > (vecthresh+1)*(long)vnodes){
        for (vseg = vsegments, vsold = (VSEGMENT FAR *)&vsegments;
            ...
            if (vseg->vs_free == &vseg->vs_data[0]){   /* empty segment */
                ...
                vsfree -= (unsigned)vseg->vs_top - (unsigned)vseg->vs_free;
    ...
    while (vdata < vfree){
        ...
        if (vector->n_type & MARK){
            if (vdata != vnext){
                ...
                MEMCPY(vnext, vdata, vsize * sizeof(LVAL));
            }
            vnext += vsize;
        }
        vdata += vsize;
...
#ifdef MIAL
    sprintf(buf,"Allocate:    %7d  Vec Allocate:%7d  Total:  %Ld\n",
