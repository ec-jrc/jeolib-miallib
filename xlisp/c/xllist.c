...
LOCAL LVAL NEAR mergesort _((LVAL list, LVAL sortfcn));

LOCAL LVAL NEAR mergesort(list,sortfcn)
LVAL list,sortfcn;
{
    /* Strategy: divide into two parts, (recurse) to sort each, then
       merge them together */
    ...
    left = mergesort(left, sortfcn);
    right = mergesort(right, sortfcn);
...
LVAL xsort()
{
    ...
    if (!null(list)) switch (ntype(list)){
        case VECTOR:
            list = listify(list);
#ifdef KEYARG
            list = null(sortkey) ? mergesort(list, sortfcn)
                                : mergesortk(list, sortfcn, sortkey);
#else
            list = mergesort(list, sortfcn);
#endif
            list = vectify(list);
            break;
        case STRING:
