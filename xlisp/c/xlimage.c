...
#ifdef MIAL
	    case IMAGE_node:
		max = GetImNByte(getimage(p));
		OSWRITE((char *)getimage(p),sizeof(IMAGE),1,fp);
		OSWRITE((char *)GetImPtr(getimage(p)),1,max,fp);
		break;
  ...
  char *fname;
{
    ...
    *xlsp++ = NIL;
	...
	case IMAGE_node:
#ifdef XDEBUG
	    (void)fprintf(stderr,"Trying to restore an image node ...");
#endif
	    p->n_info.n_image = (IMAGE *) MALLOC(sizeof(IMAGE));
	    if (p->n_info.n_image == NULL)
                xlfatal("insufficient memory - image_node");
	    if (OSREAD((char *)(p->n_info.n_image),1,sizeof(IMAGE),fp) != sizeof(IMAGE))
                xlfatal("image file corrupted (while reading an image node)");
	    max = GetImNByte(getimage(p));
            if ((p->n_info.n_image->p_im = (char *)MALLOC(max)) == NULL)
                xlfatal("insufficient memory - image_node");
            total += (long)max;
            if (OSREAD((char *)GetImPtr(getimage(p)),1,max,fp) != max)
                xlfatal("image file corrupted (while reading an image node)");
	    break;
#endif
#ifdef BIGNUMS
...
#ifdef MIAL
	case IMAGE_node:
            if (GetImPtr(getimage(p))!=NULL)
         /* old       MFREE((char *)GetImPtr(getimage(p))); old */
	      free_image(getimage(p));
            break;
#endif
        case STREAM:
