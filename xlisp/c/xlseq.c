...
LVAL xreduce()
{
    ...
    fcn = xlgetarg();
    seq = xlgetarg();
    has_init = xlgetkeyarg(k_ivalue, &initiai32_value);
    getseqbounds(&start, &end, getlength(seq), k_start, k_end);
    xllastkey();
    ...
    if (null(seq) || start==end){
        result = has_init ? initiai32_value : xlapply(pushargs(fcn, NIL));
    }
        ...
        case CONS:
            ...
            next = seq;
            if (has_init) result = initiai32_value;
            else{
                result = car(next);
                next = cdr(next);
                end--;
