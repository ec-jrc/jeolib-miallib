...
char FAR *s; int radix;
{
    ...
    *v++ = (BIGNUMDATA) sign;
    size--;
    while((i=*s++)!=0){
        if (ISLOWER7(i)) i = TOUPPER(i);    /* PSMIAL: was toupper */
...
LOCAL BIGNUMDATA andfcn(x, y)
BIGNUMDATA x,y;
{
    return x & y;
}

LOCAL BIGNUMDATA iorfcn(x, y)
BIGNUMDATA x,y;
{
    return x | y;
}

LOCAL BIGNUMDATA xorfcn(x, y)
BIGNUMDATA x,y;
{
    return x ^ y;
}

LOCAL BIGNUMDATA eqvfcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA) ~(x ^ y);
}

LOCAL BIGNUMDATA nandfcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA) ~(x & y);
}

LOCAL BIGNUMDATA norfcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA) ~(x | y);
}

LOCAL BIGNUMDATA andc1fcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA)((~x) & y);
}

LOCAL BIGNUMDATA andc2fcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA)(x & (~y));
}

LOCAL BIGNUMDATA orc1fcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA)((~x) | y);
}

LOCAL BIGNUMDATA orc2fcn(x, y)
BIGNUMDATA x,y;
{
    return (BIGNUMDATA)(x | (~y));
}
